...
class Generator(Model):
    def __init__(self, feature_dims, z_dims, hidden_size, time_step, previous_visit, predicted_visit):
        ...
        self.hidden_size = hidden_size
        self.time_step = time_step
        self.previous_time = previous_visit
...
def train_step(hidden_size, n_disc, learning_rate, l2_regularization, imbalance_kl, z_dims, t_imbalance):
    ...
    with tf.GradientTape(persistent=True) as gen_tape, tf.GradientTape(persistent=True) as disc_tape, tf.GradientTape() as encode_tape:
        ...
        while train_set.epoch_completed < epochs:
            ...
            # print(mse)
            loss = mae_loss + kl_loss_all * imbalance_kl
            ...
            gradient_generator = gen_tape.gradient(loss, variables)
...
if __name__ == '__main__':
    ...
    # mse_all = []
    # for i in range(20):
    #     mse = train_step(hidden_size=16, n_disc=4, learning_rate=0.09298193191970415, l2_regularization=0.011865473829958421, imbalance_kl=0.00012488913942561481,z_dims=16)
    #     mse_all.append(mse)
    ...
    mse_all = []
    for i in range(50):
        mse = train_step(hidden_size=16, n_disc=2, learning_rate=0.009672371940153232, l2_regularization=0.0019604325924855045,imbalance_kl=0.0004172294828816019, z_dims=16)
        mse_all.append(mse)
        print('第{}次测试完成'.format(i))
    print('----------------mse_average:{}----------'.format(np.mean(mse_all)))
    print('----------------mse_std:{}----------'.format(np.std(mse_all)))
