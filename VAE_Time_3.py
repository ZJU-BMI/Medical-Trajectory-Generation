...
def train_step(hidden_size, learning_rate, l2_regularization, imbalance_kl, z_dims, n_disc, t_imbalance):
    ...
    print('----batch_size{}---hidden_size{}----epochs{}---n_disc{}---'
          '---learning_rate{}---l2_regularization{}---kl_imbalance{}---z_dims{}---'
          .format(batch_size, hidden_size, epochs, n_disc, learning_rate, l2_regularization,
                  imbalance_kl, z_dims))
...
if __name__ == '__main__':
    ...
    # mse_all = []
    # for i in range(50):
    #     mse = train_step(hidden_size=16, n_disc=16, lambda_balance=1, learning_rate=0.1, l2_regularization=0.00001,
    ...
    mse_all = []
    for i in range(50):
        mse = train_step(hidden_size=16, n_disc=10, learning_rate=0.04138999449445984, l2_regularization=0.0004287445472435093,imbalance_kl=0.000050304181727991906, z_dims=16)
        mse_all.append(mse)
        print('第{}次测试完成'.format(i))
    print('----------------mse_average:{}----------'.format(np.mean(mse_all)))
    print('----------------mse_std:{}----------'.format(np.std(mse_all)))
    ...
    # mse_all = []
    # for i in range(50):
    #     mse = train_step(hidden_size=32, learning_rate=0.015418464179516957, l2_regularization=0.0013361721833203867,
    #                      z_dims=32, imbalance_kl=0.0019014888152403095)
    #     mse_all.append(mse)
    #     print('第{}次测试完成'.format(i))
    # print('----------------mse_average:{}----------'.format(np.mean(mse_all)))
    # print('----------------mse_std:{}----------'.format(np.std(mse_all)))