...
def train(hidden_size, learning_rate, l2_regularization):
    ...
    validate_set = np.load('../../Trajectory_generate/dataset_file/HF_validate_.npy').reshape(-1, 6, 30)[:, :, 1:]
    ...
    for k in range(1):
        ...
        # test_set = validate_sets[k]
        # train_set = DataSet(train_set)

        # 测试时采用
        train_set = DataSet(train_sets)
            ...
            with tf.GradientTape() as tape:
                ...
                if train_set.epoch_completed % 1 == 0 and train_set.epoch_completed not in logged:
                    ...
                    if (train_set.epoch_completed + 1) % 10 == 0:
                        print('------validate_num__{}---epoch{}------mse_loss{}----predicted_mse----mae_predicted---{}-'
                              '{}---predicted_r_value---{}--count  {}'.format(k, train_set.epoch_completed,
...
if __name__ == '__main__':
    test_test('AED_—HF_test__1_1.txt')
    ...
    for i in range(50):
        mse, mae, r_value, p_value = train(hidden_size=32,
                                           learning_rate=0.0005525619574929102,
                                           l2_regularization=0.0029671349213822758)
        mse_all.append(mse)
